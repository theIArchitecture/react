...
function Invoke-AdoGovernanceRemediationCommit {
    ...
    Provides remediation-branch in context for ado-create-pr to use as pr-head.
    #>
    $payload    = [Console]::In.ReadToEnd() | ConvertFrom-Json -Depth 20
    $ctx        = $payload.context
    $cfg        = $payload.config
...
function Invoke-AdoCreatePR {
    ...
    if ([string]::IsNullOrEmpty($prTitle)) {
        $errors.Add('ado-create-pr: pr-title config key not set')
        @{ success = $false; context = @{}; error = $errors -join '; '; warnings = $warnings.ToArray() } | ConvertTo-Json -Depth 10
        return
    }
    if ([string]::IsNullOrEmpty($prBase)) {
        $errors.Add('ado-create-pr: pr-base config key not set')
        @{ success = $false; context = @{}; error = $errors -join '; '; warnings = $warnings.ToArray() } | ConvertTo-Json -Depth 10
        return
    }

    # Fall back to remediation-branch written by governance-remediation-commit-ado
    if ([string]::IsNullOrEmpty($prHead) -and $ctx.'remediation-branch') { $prHead = $ctx.'remediation-branch' }
    if ([string]::IsNullOrEmpty($prHead)) {
        $errors.Add('ado-create-pr: pr-head not set and no head_ref or remediation-branch in context')
        @{ success = $false; context = @{}; error = $errors -join '; '; warnings = $warnings.ToArray() } | ConvertTo-Json -Depth 10
        return
    }
    ...
    if ([string]::IsNullOrEmpty($prBody)) {
        ...
        $errCount       = @($violations | Where-Object { $_.severity -in @('Error', 'Fatal') }).Count
        $wrnCount       = @($violations | Where-Object { $_.severity -eq 'Warning' }).Count
        $prBody = "## IArchitecture Auto-Generated PR`n`n**Violations addressed:** $violationCount ($errCount fatal, $wrnCount warning)`n`n*Generated by IArchitecture*"
    }

    # ADO requires full ref names: refs/heads/<branch>
    ...
    if ($prLabels) {
        $body['labels'] = @($prLabels -split ',' | ForEach-Object { @{ name = $_.Trim() } })
    }
    ...
    try {
        ...
        # Reconstruct the PR URL from org/project/repo info in ci-environment
        ...
        $errors.Add("ado-create-pr: failed to create PR - $_")
    }

    @{
        success  = ($errors.Count -eq 0)
        context  = @{ 'pr-url' = $prUrl }
        error    = if ($errors.Count -gt 0) { $errors -join '; ' } else { $null }
        warnings = $warnings.ToArray()
    } | ConvertTo-Json -Depth 10
}
